...
async function handleSubscriptionActivated(event) {
  ...
  // 使用辅助函数从 PayPal Plan ID 推断套餐类型和计费周期
  const planInfo = await getMemberPlanFromPayPalPlanId(planId);
  const memberPlan = planInfo.plan;
  const suggestedBillingPeriod = planInfo.period; // 从 Plan 推断的计费周期
  
  console.log(`   ✅ 推断结果: memberPlan="${memberPlan}", billingPeriod="${suggestedBillingPeriod}"`);
  ...
  try {
    ...
    `;
    const userData = await mutateHasura(userQuery, { userId });
  ...
  try {
    ...
    if (paymentResult.insert_payment_one && paymentResult.insert_payment_one.id) {
      console.log(`✅ Recorded payment transaction (ID: ${paymentResult.insert_payment_one.id}) for subscription activation${affiliateId ? ` (affiliate: ${affiliateId})` : ''}`);
    } else {
