 ...
 * @param {string} ourTankId - Optional Our Tank ID for friend group chats
 * @returns {Promise<string>} - Session ID
 */
async function saveGroupChatSession(chatResult, fishArray, initiatorUserId = null, ourTankId = null) {
    const mutation = `
    ...
    const variables = {
        ...
        our_tank_id: ourTankId || null  // Our Tank ID for friend group chats
    };
    ...
    console.log('[AI Fish Group Chat] Saving group chat session with variables:', {
        ...
        our_tank_id: variables.our_tank_id,
    ...
    try {
        ...
        // 同时保存到 conversations 表，并获取 conversation_id (UUID)
        // 然后更新 group_chat 记录的 conversation_id 字段
        if (savedSession.coze_conversation_id) {
            try {
                const conversationUuid = await saveConversationRecord({
                ...
                // 如果成功获取到 conversation_id (UUID)，更新 group_chat 记录
                if (conversationUuid) {
                    const updateMutation = `
                        mutation UpdateGroupChatConversationId($id: uuid!, $conversation_id: uuid!) {
                    ...
                    if (updateResult.errors) {
                        console.warn('[AI Fish Group Chat] ⚠️ Failed to update conversation_id:', updateResult.errors);
                    } else {
                        console.log('[AI Fish Group Chat] ✅ Updated group_chat with conversation_id:', conversationUuid);
                    }
                }
            } catch (error) {
...
const mainHandler = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            // Check for Our Tank ID (for friend group chats)
            if (body && body.ourTankId) {
                req.ourTankId = body.ourTankId;
                console.log('[AI Fish Group Chat] Our Tank ID provided:', body.ourTankId);
            }
        }
        ...
        // Save to database with initiator user ID and optional Our Tank ID
        let sessionId = null;
        const ourTankId = req.ourTankId || null;
        try {
            console.log('[AI Fish Group Chat] Saving session with initiator_user_id:', requestUserId, 'our_tank_id:', ourTankId);
            sessionId = await saveGroupChatSession(chatResult, fishArray, requestUserId, ourTankId);
            console.log('[AI Fish Group Chat] ✅ Session saved successfully:', sessionId, 'with initiator_user_id:', requestUserId, 'our_tank_id:', ourTankId);
