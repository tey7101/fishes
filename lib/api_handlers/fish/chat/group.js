 ...
 * Priority: 1) userLanguageFromRequest, 2) initiator's language, 3) first non-empty user_language from fish array, 4) default to "English"
 ...
 * @param {string} userLanguageFromRequest - Optional language preference from frontend request
 * @returns {Promise<string>} - Language value directly from user setting (e.g., "简体中文", "English")
 */
async function determineGroupChatLanguage(fishArray, initiatorUserId = null, userLanguageFromRequest = null) {
    // First priority: Use language from frontend request (most up-to-date)
    if (userLanguageFromRequest && userLanguageFromRequest.trim()) {
        console.log('[AI Fish Group Chat] Using language from frontend request:', userLanguageFromRequest);
        return userLanguageFromRequest.trim();
    }
    
    // Second priority: try to get initiator's language preference from database
    if (initiatorUserId) {
        try {
            const userQuery = `
    ...
    // Fallback: Find the first non-empty user_language from fish owners
    // 注意：fishArray中不再包含user_language，需要从数据库查询fish的owner
    if (fishArray.length > 0) {
        try {
            // 获取所有鱼的user_id（从fish表查询）
            const fishIds = fishArray.map(f => f.fish_id);
            const fishQuery = `
                query GetFishOwnersLanguage($fishIds: [uuid!]!) {
            ...
            `;
            const fishResult = await executeGraphQL(fishQuery, { fishIds });
            if (fishResult.data && fishResult.data.fish) {
                for (const fish of fishResult.data.fish) {
                    if (fish.user && fish.user.user_language && fish.user.user_language.trim()) {
                        const ownerLanguage = fish.user.user_language.trim();
                        console.log('[AI Fish Group Chat] Using fish owner language:', ownerLanguage);
                        return ownerLanguage;
                    }
                }
            }
        } catch (error) {
            console.warn('[AI Fish Group Chat] Failed to get fish owner language:', error);
        }
    }
 ...
 */
async function generateGroupChat(fishArray, initiatorUserId = null, options = {}, debugInfo = null, userLanguageFromRequest = null) {
    ...
    // Determine the language to use (prioritize frontend request, then initiator's language)
    const outputLanguage = await determineGroupChatLanguage(fishArray, initiatorUserId, userLanguageFromRequest);
