...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 获取 coze_conversation_id（Coze API 返回的字符串 ID）
        let cozeConversationIdValue = result.cozeConversationId || null;
        if (cozeConversationIdValue && typeof cozeConversationIdValue !== 'string') {
            console.warn('[User Chat Message] cozeConversationId is not a string, converting:', cozeConversationIdValue);
            cozeConversationIdValue = String(cozeConversationIdValue);
        }
        ...
        const saveResult = await executeGraphQL(mutation, {
            ...
            coze_conversation_id: cozeConversationIdValue,
        ...
        const savedCozeConversationId = saveResult.data.insert_group_chat_one.coze_conversation_id;
        const savedUserTalk = saveResult.data.insert_group_chat_one.user_talk;
        
        console.log('[User Chat Message] ✅ 消息发送成功', {
            sessionId: newSessionId,
            conversationId: savedConversationId,
            cozeConversationId: savedCozeConversationId,
        ...
        if (!savedCozeConversationId && result.cozeConversationId) {
            console.warn('[User Chat Message] ⚠️ coze_conversation_id 未正确保存', {
                expected: result.cozeConversationId,
                saved: savedCozeConversationId
            });
        }
        if (!savedUserTalk) {
